...
int __init ubnt_prune_device_tree(void)
{
	...
	pr_info("UBNT board DTS pruning ... board %d major %d\n",
		octeon_bootinfo->board_type, octeon_bootinfo->major_version);

	if (octeon_bootinfo->board_type == CVMX_BOARD_TYPE_UBNT_E300) {
		// Remove unused MMC slot definition
		int mmc_slot2;
		mmc_slot2 = fdt_path_offset(initial_boot_params, "/soc/mmc/mmc-slot@2");
		if (mmc_slot2 > 0)
			fdt_nop_node(initial_boot_params, mmc_slot2);
	} else if (octeon_bootinfo->board_type == CVMX_BOARD_TYPE_UBNT_E200 || octeon_bootinfo->board_type == CVMX_BOARD_TYPE_UBNT_E220) {
		// Set frequency to 26MHZ
		int mmc_slot0;
		u32 freq = 26000000;
		mmc_slot0 = fdt_path_offset(initial_boot_params,
			"/soc/mmc/mmc-slot@0");
		if (mmc_slot0 > 0) {
