...
static inline uid_t ia_uid_read(const struct iattr *attr)
{
        return from_kuid(&init_user_ns, attr->ia_uid);
}

static inline gid_t ia_gid_read(const struct iattr *attr)
{
        return from_kgid(&init_user_ns, attr->ia_gid);
}

static inline void ia_uid_write(struct iattr *attr, uid_t uid)
{
        attr->ia_uid = make_kuid(&init_user_ns, uid);
}

static inline void ia_gid_write(struct iattr *attr, gid_t gid)
{
        attr->ia_gid = make_kgid(&init_user_ns, gid);
}
#else
